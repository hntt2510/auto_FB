...
export class DashboardService {
  ...
  constructor(private readonly db: Database.Database, private readonly settings?: PublishingSettingsService, private readonly sessions?: AccountSessionRepository) { this.queue = new QueueRepository(db); this.audit = new AuditLogRepository(db); this.publishing = new PublishRepository(db); }
  ...
  summary(): DashboardSummary {
    ...
    const onboarding = this.db.prepare("SELECT SUM(CASE WHEN onboarding_status = 'NEW' THEN 1 ELSE 0 END) AS new_count, SUM(CASE WHEN onboarding_status = 'WARMING' THEN 1 ELSE 0 END) AS warming, SUM(CASE WHEN onboarding_status = 'READY' THEN 1 ELSE 0 END) AS ready, SUM(CASE WHEN onboarding_status = 'PAUSED' THEN 1 ELSE 0 END) AS paused FROM accounts").get() as Record<string, number | null>;
    const onboardingTasks = (this.db.prepare("SELECT COUNT(*) AS count FROM account_onboarding_tasks t JOIN accounts a ON a.id = t.account_id WHERE a.onboarding_status = 'WARMING' AND t.day_number = MIN(a.onboarding_plan_days, CAST(julianday(?) - julianday(a.onboarding_started_at) AS INTEGER) + 1)").get(now) as { count: number }).count;
    const sessionMetrics = this.sessions?.dashboard(today, tomorrow, now) ?? { sessionsToday: 0, activeNow: 0, minutesToday: 0, dailyTargetsCompleted: 0 };
