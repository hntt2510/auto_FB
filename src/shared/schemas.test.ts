...
describe('account schemas', () => {
  ...
  it('defaults and bounds scheduler session and batch pacing safety settings', () => { const base = { enabled: false, executionMode: 'DRY_RUN', schedulerIntervalSeconds: 30, maxConcurrentAccounts: 2, videoUploadTimeoutSeconds: 600, canaryMode: true }; const parsed = publishingSettingsSchema.parse(base); expect(parsed.maxJobsPerSchedulerSession).toBe(20); expect(parsed.batchPacingSeconds).toBe(120); expect(publishingSettingsSchema.safeParse({ ...base, maxJobsPerSchedulerSession: 0 }).success).toBe(false); expect(publishingSettingsSchema.safeParse({ ...base, maxJobsPerSchedulerSession: 101 }).success).toBe(false); expect(publishingSettingsSchema.safeParse({ ...base, batchPacingSeconds: 9 }).success).toBe(false); expect(publishingSettingsSchema.safeParse({ ...base, batchPacingSeconds: 3601 }).success).toBe(false); });
  it('bounds account session targets, defaults legacy auto-close, and confines operator URLs to Facebook', () => { const id = '00000000-0000-4000-8000-000000000000'; expect(accountSessionSettingsSchema.parse({ targetDurationMinutes: 30 })).toEqual({ targetDurationMinutes: 30, autoCloseBrowserAfterTarget: false }); expect(accountSessionSettingsSchema.safeParse({ targetDurationMinutes: 9 }).success).toBe(false); expect(accountSessionSettingsSchema.safeParse({ targetDurationMinutes: 61 }).success).toBe(false); expect(accountSessionNavigationSchema.safeParse({ accountId: id, destination: 'URL', url: 'https://www.facebook.com/profile.php?id=1' }).success).toBe(true); expect(accountSessionNavigationSchema.safeParse({ accountId: id, destination: 'URL', url: 'https://facebook.com.evil.example/' }).success).toBe(false); expect(accountSessionNavigationSchema.safeParse({ accountId: id, destination: 'URL' }).success).toBe(false); });
});
