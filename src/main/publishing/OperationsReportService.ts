...
export type SanitizedOperationsReport = {
  ...
  publishing: { enabled: boolean; executionMode: string; canaryMode: boolean; schedulerIntervalSeconds: number; maxConcurrentAccounts: number; schedulerState: string; backlogCount: number; sessionCompleted: number; sessionLimit: number };
...
export class OperationsReportService {
  ...
  build(): SanitizedOperationsReport {
    ...
    return {
      ...
      publishing: ((value) => ({ enabled: value.enabled, executionMode: value.executionMode, canaryMode: value.canaryMode !== false, schedulerIntervalSeconds: value.schedulerIntervalSeconds, maxConcurrentAccounts: value.maxConcurrentAccounts, schedulerState: this.scheduler?.runtimeState() ?? 'DISARMED', backlogCount: this.queue.dueCount(new Date().toISOString()), sessionCompleted: this.scheduler?.completedThisSession() ?? 0, sessionLimit: value.maxJobsPerSchedulerSession ?? 20 }))(this.settings.get()),
