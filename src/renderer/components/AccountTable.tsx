...
export function AccountTable({ accounts, loading, busy, onOpen, onClose, onHealth, onEdit, onDelete, onOpenFolder }: Props) {
  ...
  return <div className="table-card"><div className="table-wrap"><table><thead><tr><th>Account</th><th>Profile</th><th>Network</th><th>Status</th><th>Last opened</th><th>Last health check</th><th>Actions</th></tr></thead><tbody>
    {accounts.map((account) => { const action = busy[account.id]; const running = account.status === 'RUNNING' || account.status === 'STARTING'; return <tr key={account.id}>
      ...
      <td><div className="network-cell"><span className={`network-dot ${account.proxyEnabled ? 'proxy' : ''}`}></span><span>{account.proxyEnabled ? 'Proxy' : 'Direct'}</span>{account.proxyEnabled && <small>{maskProxy(account.proxyHost, account.proxyPort)}</small>}</div></td>
