...
- **Pacing & Workload Control:** A fixed per-account cooldown (default **120 seconds**, configurable from 10 to 3600 seconds) begins after the complete publishing attempt finishes (including the 8-second post-submit observation window). Pacing controls workload burst; it is not a guarantee against Facebook platform restrictions.
