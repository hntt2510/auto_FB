...
export class DraftRepository {
  ...
  hasActiveQueueItems(id: string): boolean { return Boolean(this.db.prepare("SELECT 1 FROM queue_items WHERE draft_id = ? AND status IN ('PENDING', 'PAUSED') LIMIT 1").get(id)); }
