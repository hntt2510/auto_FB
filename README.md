...
- **Planning (DRAFT):** Define a campaign, attach multiple Drafts as content variants (e.g. Variant A, Variant B), and construct targeted plan items linking each variant to specific account/group pairs with optional UTC schedule times. Campaigns can only be deleted while in `DRAFT` status and only when zero Queue items (active or historical) reference them.
...
- **Approval (APPROVED):** Approving a campaign captures exact cryptographic snapshot hashes (`buildSnapshotHash`) of all enabled variants and their underlying draft title, body, link, and ordered media assets. Any subsequent modification to an approved draft immediately flags the campaign as `APPROVAL_STALE`, blocking queue materialization. Operators can explicitly reopen stale or approved campaigns for changes via `Reopen for Changes` (`APPROVED` → `DRAFT`), clearing approval hashes and requiring a full re-review and fresh approval snapshot before commit.
- **Simulation (READ-ONLY):** Runs a comprehensive verification of account assignments, active group status, draft readiness, approval freshness, intra-campaign duplicate collisions, and schedule conflicts. Intra-campaign duplicate rows sharing the Queue-equivalent identity (`draftId + snapshotHash + accountId + groupId + scheduledAt`) are classified as `BLOCKER` issues (`status: 'BLOCKED'`), preventing commit. Schedule conflicts (within 15 minutes of existing `PENDING`/`PAUSED` queue rows or intra-campaign rows on the same account) are surfaced as Planner-aligned warnings. Simulation creates zero Queue rows and performs zero Facebook or browser operations.
...
- **Provenance & Lineage:** Committed queue rows retain permanent linkage via `campaign_id` and `campaign_variant_id`. Requeuing a campaign queue item preserves this lineage across all terminal states, while manual/legacy queue items remain unlinked and fully compatible.
