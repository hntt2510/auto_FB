...
type Props = { accounts: FacebookAccount[]; loading: boolean; busy: Record<string, string>; onOpen: (id: string) => void; onClose: (id: string) => void; onHealth: (id: string) => void; onEdit: (account: FacebookAccount) => void; onDelete: (account: FacebookAccount) => void; onWarmup: (account: FacebookAccount) => void; onOpenFolder: (id: string) => void };

export function AccountTable({ accounts, loading, busy, onOpen, onClose, onHealth, onEdit, onDelete, onWarmup, onOpenFolder }: Props) {
  ...
  return <div className="table-card"><div className="table-wrap"><table><thead><tr><th>Account</th><th>Profile</th><th>Network</th><th>Proxy</th><th>Session</th><th>Warm-up</th><th>Day / last session</th><th>Actions</th></tr></thead><tbody>
